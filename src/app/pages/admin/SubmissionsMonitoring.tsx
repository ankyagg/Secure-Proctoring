...
import { RefreshCw, Search, Filter, Download, CheckCircle2, XCircle, Clock, Zap, Code2 } from "lucide-react";
import { adminSubmissions } from "../../data/mockData";

type Submission = typeof adminSubmissions[0];
...
// Fake new submissions that come in "live"
const liveQueue: Submission[] = [
...
export default function SubmissionsMonitoring() {
  ...
  const rejectedCount = submissions.filter((s) => s.verdict !== "Accepted").length;
    ...
    <div className="p-6 space-y-5">
      ...
      <div className="grid grid-cols-4 gap-3">
        {[
          ...
          {
            label: "Accept Rate",
            value: `${Math.round((acceptedCount / submissions.length) * 100)}%`,
            color: "text-blue-700",
          },
        ].map((item) => (
          <div key={item.label} className="bg-white border border-slate-200 rounded-xl p-4">
            <div className={`text-xl mb-0.5 ${item.color}`} style={{ fontWeight: 700 }}>
              {item.value}
            </div>
        ...
        <div className="divide-y divide-slate-100">
          {filtered.map((sub, idx) => {
            ...
            return (
              <div
                key={sub.id}
                className={`grid px-5 py-3.5 items-center hover:bg-slate-50/50 transition-colors ${
                  idx === 0 && liveEnabled && liveIndex > 0 ? "bg-blue-50/40" : ""
                }`}
              ...
              >
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-slate-700 text-xs truncate" style={{ fontWeight: 500 }}>
                    {sub.user}
                  </span>
                </div>
                ...
                <span className="text-slate-400 text-xs" style={{ fontFamily: "monospace" }}>
                  {sub.timestamp}
                </span>
              </div>
            );
